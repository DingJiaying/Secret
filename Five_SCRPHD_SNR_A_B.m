...
w = 0.3 * pi;   %从pi/25到pi-pi/25每隔pi/25取一个
B =1;
AA = 0:0.02:2; 
phiA = 0.1*pi;
phiB = 0.2*pi;
...
for A = AA 
    ...
    for t = 1:T
        n = 1:1:N;
        sn = (A+B)*exp(- 1j*phiA)*exp(1j*w*n) +  B*exp(- 1j*phiB)*exp(-1j*w*n);
...
plot(AA, MSE_SCRPHD1, '-', 'LineWidth', 2);
hold on;
plot(AA, MSE_SCRPHD2, '-', 'LineWidth', 2);
plot(AA, MSE_SCRPHD3, '-', 'LineWidth', 2);
plot(AA, MSE_SCRPHD4, '-', 'LineWidth', 2);
